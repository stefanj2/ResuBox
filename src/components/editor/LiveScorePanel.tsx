...
export function LiveScorePanel() {
  ...
  return (
    <>
      {/* Floating pill — bottom-right of viewport */}
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      >
        ...
        <div className="text-left">
          <p className="text-[10px] uppercase tracking-wider text-slate-400 leading-none mb-0.5">CV-score</p>
        ...
        <>
          ...
          <div className="hidden sm:flex flex-col fixed bottom-20 right-5 z-40 w-[380px] max-h-[70vh] bg-white rounded-2xl border border-slate-200 shadow-2xl overflow-hidden">
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-2">
                    {score.metadata.wordCount} woorden · {score.metadata.bulletCount} bullets ·{' '}
                    {score.metadata.quantifiedBullets} met cijfers
                  </p>
                </div>
                <button
                  onClick={() => setOpen(false)}
            ...
            <div className="px-5 py-3 bg-slate-50 border-t border-slate-100 text-xs text-slate-500">
              Realtime check — geen AI-oordeel, alleen regels die recruiters daadwerkelijk hanteren.
            </div>
          </div>
        </>
      )}
...
// Suppress unused import warning for Gauge — kept for future iteration
void Gauge;
